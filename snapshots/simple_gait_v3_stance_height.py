...
def compute_targets(t):
    ...
    feet[:, 2] += current_stance_height(t)        # stance height applied first
...
def main():
    ...
    if CYCLE_STANCE_HEIGHTS:
        lo_mm = min(STANCE_HEIGHT_CYCLE_M) * 1000
        hi_mm = max(STANCE_HEIGHT_CYCLE_M) * 1000
        stance_descr = (f"cycling {len(STANCE_HEIGHT_CYCLE_M)} stance heights "
                        f"({lo_mm:+.1f}…{hi_mm:+.1f} mm) every {STANCE_DWELL_SECONDS}s")
    else:
        stance_descr = f"STANCE_HEIGHT_OFFSET={STANCE_HEIGHT_OFFSET*1000:+.1f}mm"
    print(f"\nsimple_gait | MODE={descr}  {head_descr}  {stance_descr}  "
          f"PATH_RADIUS={PATH_RADIUS}  PERIOD={GAIT_PERIOD}s")
