...
#   "spin"   : spin in place. Each foot traces an arc on its own circle around
#              body origin; same angular sweep Δθ for every leg → coherent body
#              rotation, no slipping. Tripod phase still applies (alternating
#              support triangles). Sign of SPIN_DIRECTION flips CW vs CCW.
MODE        = "spin"
...
# ----------------------------------------------------------------------------
# SPIN — when MODE=="spin", each foot traces an arc on its own concentric
# circle around body origin. Path's x_path is interpreted as an angular sweep:
#   Δθ = SPIN_DIRECTION * x_path / SPIN_REFERENCE_RADIUS
# Same Δθ for every leg → coherent body rotation. Sign convention (verified
# in mujoco): SPIN_DIRECTION = +1 → body spins CCW in world (positive yaw,
# RH around +Z, looking down +Z);  -1 → body spins CW in world.
# Per gait cycle the body rotates by 4·Δθ_max with default settings:
#   Δθ_max = PATH_RADIUS / SPIN_REFERENCE_RADIUS
#          = 0.025 / 0.204 = 0.1225 rad ≈ 7.02°
#   per cycle (T=1.5s) ≈ 28°  →  ~18.7°/s commanded  →  measured ~17.5°/s in
#   sim (some loss to PD compliance + floor friction). Full 360° in ~21 s.
# ----------------------------------------------------------------------------
SPIN_REFERENCE_RADIUS = 0.204    # m. Corner radius (recommended). See math docs.
SPIN_DIRECTION        = +1       # +1 / -1; static value used when CYCLE_SPIN is False

# Spin direction cycle — alternates spin direction with various dwell times so
# you can verify the bot completes 360°+ rotations consistently in both
# directions over many cycles. Each entry is (sign, duration_seconds).
# Default rotations land roughly: 22s≈385°, 25s≈438°, 20s≈350°, 23.5s≈412°
# (measured in sim, slightly less than naive 18.7°/s × dur due to compliance).
CYCLE_SPIN = True
SPIN_CYCLE = [
    (+1, 22.0),     # CCW in world for 22s  ≈ 385°  (~25° past 360°)
    (-1, 25.0),     # CW  in world for 25s  ≈ 438°  (~78° past 360°)
    (+1, 20.0),     # CCW in world for 20s  ≈ 350°  (just shy of full turn)
    (-1, 23.5),     # CW  in world for 23.5s ≈ 412° (~52° past 360°)
]
...
_spin_cycle_state = {"last_idx": -1}

def current_spin_direction(t):
    """Return the spin direction sign (+1 or -1) for sim time t. When CYCLE_SPIN
    is True, walks through SPIN_CYCLE entries (sign, duration_s). Cycles the
    full list and repeats. Otherwise returns the static SPIN_DIRECTION."""
    if not CYCLE_SPIN:
    ...
    for idx, (sign, dur) in enumerate(SPIN_CYCLE):
        if t_in_cycle < cumulative + dur:
            if idx != _spin_cycle_state["last_idx"]:
                # Approximate degrees swept per phase = (4·Δθ_max/T) · dur
                deg_per_sec = math.degrees(4 * (PATH_RADIUS / SPIN_REFERENCE_RADIUS)) / GAIT_PERIOD
                approx_deg  = deg_per_sec * dur
                world_dir   = "CCW" if sign > 0 else "CW "
                print(f"  t={t:5.1f}s  spin → world {world_dir} for {dur:.1f}s  "
                      f"(~{approx_deg:.0f}° commanded)")
                _spin_cycle_state["last_idx"] = idx
            return sign
        cumulative += dur
...
def compute_targets(t):
    ...
    if MODE == "stand":
        ...
        sign = current_spin_direction(t)
        for i in range(6):
            s_i = (s_global + LEG_PHASE[i]) % 1.0
            px, pz = path(s_i)
            dtheta = sign * px / SPIN_REFERENCE_RADIUS
            # Each foot's polar position around body origin (uses width-adjusted
            # origin already in `feet[i]`, so stance-width changes still apply).
            x0, y0 = feet[i, 0], feet[i, 1]
            r_i    = math.hypot(x0, y0)
            th_i   = math.atan2(y0, x0)
            new_th = th_i + dtheta
            feet[i, 0] = r_i * math.cos(new_th)
            feet[i, 1] = r_i * math.sin(new_th)
            feet[i, 2] += pz                            # z lift (height already in feet[i,2])
...
def main():
    ...
    if MODE == "spin":
        if CYCLE_SPIN:
            spin_descr = (f"spin cycle: " +
                          ", ".join(f"{'CCW' if s>0 else 'CW'}/{d:.0f}s" for s,d in SPIN_CYCLE))
        else:
            spin_descr = f"static spin direction = {'CCW' if SPIN_DIRECTION>0 else 'CW'}"
    else:
        spin_descr = None
    ...
    print(f"             PATH_RADIUS={PATH_RADIUS}  PERIOD={GAIT_PERIOD}s  "
          f"SPIN_REF_R={SPIN_REFERENCE_RADIUS}m")
