"""
HexapodEnv — macOS variant of the PhantomX hexapod RL environment.

Identical to envs/hexapod_env.py (the Linux/workstation version) except for
how the live training viewer works:
  * Linux env opens a Cocoa-style passive viewer inside the SubprocVecEnv
    worker process. That works because Linux has no main-loop restriction.
  * macOS can't do that — the Cocoa viewer requires the calling process to
    be `mjpython`, but SubprocVecEnv workers run as plain `python`. So this
    Mac variant publishes env-0's qpos+qvel+sim_time to a shared-memory
    region every step, and a separate `mjpython live_viewer.py` process reads
    that region and renders.

All other behavior — cmd vector layout, reward, curriculum stages, drift
penalty, polar-sampled translation, all-overlays-in-stage-1 mask, etc. — is
identical to (or improvements over) the Linux version. The Linux env on the
workstation may not have these improvements yet; merge as needed.
