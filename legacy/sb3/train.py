"""
train.py — PPO training entry point. Cross-platform (macOS + Linux).

Imports the unified `envs.hexapod_env`. Live-viewer mechanism is shared
across platforms via shared memory: env-0 publishes its qpos+qvel+sim_time
every step to a named SHM region; a separate `live_viewer.py` process
reads it and renders. Run that viewer with:
  * macOS:  mjpython live_viewer.py
  * Linux:  python   live_viewer.py

CLI flags:
  --resume [path]      Continue from an existing checkpoint. With no path,
                       picks the latest in CKPT_DIR. Resume mode forces
                       gait_scale=0.0 (skips scaffold fade).
  --bc-init <path>     Load BC-pretrained policy weights as initialization
                       for fresh RL. Resets log_std to a safer default.
  --log-std-init <f>   Override log_std after weight load. -2.0 default
                       under --bc-init; -3.0 recommended for refinement.

Edit RUN_NAME / TOTAL_STEPS / INITIAL_STAGE / SKIP_SCAFFOLD / schedule
constants below as needed.
"""
...
# CONFIG
# ============================================================================
RUN_NAME    = "simple_model_5m_validation"
LOG_DIR     = f"logs/hexapod_{RUN_NAME}"
CKPT_DIR    = f"checkpoints/hexapod_{RUN_NAME}"
N_ENVS      = 32
# Validation run: 5M from-scratch curriculum on the primitive-geom
# phantomx_simple.xml model. Goal is to confirm the simplified MJCF
# trains end-to-end like the mesh model does, with no BC-init dependency
# and no permanent edits required. Compressed schedule (1M / 1M / 1.5M /
# autonomous) mirrors the proportions of a longer run, just shrunk to fit
# 5M total. After validation, revert RUN_NAME / TOTAL_STEPS / INITIAL_STAGE
# / SKIP_SCAFFOLD / STAGE_MIN_STEPS / MODEL_PATH (in env factory below)
# back to v6 BC-init refinement settings.
TOTAL_STEPS = 5_000_000

# From-scratch curriculum (4-stage with scaffold fade) — cleanest test of
# the simplified model end-to-end.
INITIAL_STAGE   = 1
SKIP_SCAFFOLD   = False

# Override the env's MJCF to the primitive-geom simplified model.
# When None, env uses its built-in default (mesh-based phantomx.xml).
MODEL_PATH_OVERRIDE = "models/phantomx_simple.xml"
...
# Per-stage minimum step count. Compressed for the 5M validation run
# (proportional to the full 100M-curriculum schedule, just 1/20 scale).
STAGE_MIN_STEPS = {
    1:   1_000_000,   # ≥ stage 1 plateau point — scaffold-strong baseline
    2:   1_000_000,   # scaffold fade 1.0 → 0.6, +wz/height/width cmds
    3:   1_500_000,   # scaffold fade 0.6 → 0.0, +pitch/roll cmds
    4:           0,   # remaining ~1.5M of autonomous training
}
...
STAGE_FADE_RANGE = (0.30, 0.90)   # fade between 30% and 90% of each stage's
                                   # nominal duration (matches the original
                                   # general-purpose curriculum default)
...
def _make_env(idx, watch_live):
    ...
    if MODEL_PATH_OVERRIDE:
        kwargs["model_path"] = MODEL_PATH_OVERRIDE
